				...
				<div className="flex items-center gap-4 flex-1 min-w-0">
					...
					<div className="flex flex-1 items-center gap-2 scrollbar-hide overflow-auto">
						...
						{minPrice != undefined && maxPrice == null && (
							<Chip onClick={clearPriceRange}>
								{t("productsPage.above")} {formatPrice(minPrice / 100, locale)}
							</Chip>
						)}
						{minPrice == null && maxPrice != undefined && (
							<Chip onClick={clearPriceRange}>
								{t("productsPage.under")} {formatPrice(maxPrice / 100, locale)}
							</Chip>
						)}
						...
						{rating !== undefined && rating === 4 && (
							<Chip onClick={clearRating}>4.0+ {t("productsPage.rating")}</Chip>
						)}
						{rating !== undefined && rating === 3 && (
							<Chip onClick={clearRating}>3.0+ {t("productsPage.rating")}</Chip>
						)}
						{rating !== undefined && rating === 2 && (
							<Chip onClick={clearRating}>2.0+ {t("productsPage.rating")}</Chip>
						)}
						{rating !== undefined && rating === 1 && (
							<Chip onClick={clearRating}>1.0+ {t("productsPage.rating")}</Chip>
						)}
					</div>
				</div>
